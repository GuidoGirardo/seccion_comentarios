<?php
    ...
    if(isset($_POST['postear_comentario'])){
        ...
        $sql = "INSERT INTO video2 (nombre, comentario) VALUES ('$nombre', '$comentario')";
        if($conn->query($sql) === true) header("Location: http://localhost/xampp/scomentarios/pagina2.php");
            ...
            <?php
                ...
                if($result->num_rows > 0){
                    while($row = $result->fetch_assoc()){
                        ...
                        echo "<button class='btnLike' video='video2' data-id='{$row['id']}'>LIKE</button>";
                        echo "<button class='btnDislike' video='video2' data-id='{$row['id']}'>DISLIKE</button>";
                        echo "</div>";
                    }
                }
            ?>
